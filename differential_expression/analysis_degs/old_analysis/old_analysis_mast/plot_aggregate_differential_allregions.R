...
gplot = ggplot(pltdf, aes(coef, -log10(pvalue), color=factor(col))) + 
    geom_point(cex=.5) + 
    geom_text_repel(data=labdf, aes(label=primerid), size=2.5, segment.size=.25, max.overlaps=15) + 
    theme_pubr() + 
...
ggsave(paste0(imgpref, 'mastlmm_volcano_', path, '.allreg.major.', celltype, '.minor.', ststr,'.png'), gplot, dpi=400, units='in', width=6,height=6)
ggsave(paste0(imgpref, 'mastlmm_volcano_', path, '.allreg.major.', celltype, '.minor.', ststr,'_small.png'), gplot, dpi=400, units='in', width=5,height=5)
