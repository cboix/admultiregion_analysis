...
for (subtype in subtypes){
    ...
    for (path in c(pathlist,'cogdxad', 'nrad')) {
        ...
        # mdx = model.matrix(asform(c('~',path, '+ region + age_death + msex + pmi')), data=sub.pathdf)
        if (reg == 'All'){
            mdx = model.matrix(asform(c('~',path, '+', path,'* region + age_death + msex + pmi + cell_type_high_resolution')), data=sub.pathdf)
            m2 = model.matrix(asform(c('~',path, '+ region + age_death + msex + pmi + cell_type_high_resolution')), data=sub.pathdf)
        } else {
