...
# Raw counts of degs
# Number of overlaps across region/condition
# Updated: 03/22/22
# --------------------------------------------------------
library(cbrbase)
...
# Jaccard overlap for each set in each path separately:
# -----------------------------------------------------
selrundf = rundf[rundf$path == 'nrad',]
selrundf$setid = with(selrundf, paste0(celltype, '_', subtype))
sets = unique(selrundf$setid)
sets = c(sets[grep("^Exc_",sets, invert=TRUE)], 'Exc_Exc')
selrundf = selrundf[selrundf$setid %in% sets,]
...
for (path in pathlist){
    ...
    alldf = alldf[alldf$col_nm != 0,]
    rn = c('allregions', 'EC','HC','TH','AG','MT','PFC')
    genesets = lapply(rn, function(x){
        ind = alldf$region == x
        if (sum(ind) == 0){
            gene = NULL
        } else {
            gene = paste0(alldf$gene[ind], '-', alldf$col[ind], '@', alldf$set)
        }
        return(gene)
            })
    names(genesets) = rn
    ...
    # Now compute for each set separately, keeping the rn ordering:
    jlist = list()
    mx = 0
    for (set in sets){
        ...
        genesets = lapply(rn, function(x){
            ...
            if (sum(ind) == 0){
                ...
                gene = paste0(fulldf$gene[ind], fulldf$col[ind])
            }
            return(gene)
            })
        names(genesets) = rn
        ...
        if (N > 1){
            ...
            colnames(jmat) = names(genesets)
            jmat = jmat[rn,rn]
            uqvals = jmat[upper.tri(jmat)]
            cat(set, '\t', round(100 * mean(uqvals),2), '\n')
            jlist[[set]] = jmat
            mx = max(c(mx, uqvals), na.rm=T)
        }
    }


    # Plot each of these jaccard overlap matrices:
    # --------------------------------------------
    ux = 1.5
    reg.cols['allregions'] = 'grey75'
    for (set in sets){
        jmat = jlist[[set]]
        jmat = jmat - diag(diag(jmat) * NA)

        # Add an annotation based on the names:
        ha = HeatmapAnnotation(
            Region=rownames(jmat),
        ...
        col_fun = colorRamp2(seq(0,mx,length.out=length(colspec)), rev(colspec))
        plt = Heatmap(jmat, 
        ...
        w = 5 + 1 / 15 * ncol(jmat)
        pltprefix = paste0(imgpref, 'allmethods_jaccard_ovl.', path, '.set_', set, '.heatmap')
        saveHeatmap(plt, pltprefix, w=w, h=h)
    }
}
