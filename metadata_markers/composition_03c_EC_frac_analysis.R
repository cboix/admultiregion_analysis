...
# ct = 'Exc TOX3 TTC6'
# Significance from wilcoxon; greater, adjusted:
cts = unique(ctdf$celltype)
pv = c()
for (ct in cts){
    x1 = ctdf$frac[ctdf$celltype == ct & ctdf$nrad == 'AD']
    x2 = ctdf$frac[ctdf$celltype == ct & ctdf$nrad == 'CTRL']
    pv = c(pv, wilcox.test(x1,x2, alternative ='less')$p.value)
}
names(pv) = cts
pv = pv[order(pv)]
padj = p.adjust(pv, 'fdr')
labdf = data.frame(celltype=names(padj), padj=padj)
